...
class InvertedIndex:  
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl, bucket_name, folder_str):
        posting_locs = defaultdict(list)
        bucket_id, list_w_pl = b_w_pl
        writer = MultiFileWriter("." + folder_str, bucket_id, bucket_name)
        for w, pl in list_w_pl:
            # convert to bytes
            b = b''.join([(doc_id << 16 | (tf & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
                          for doc_id, tf in pl])
            # write to file(s)
            locs = writer.write(b, folder_str)
            # save file locations to index
            posting_locs[w].extend(locs)
        writer.close()
        writer.upload_to_gcp()
        InvertedIndex._upload_posting_locs(bucket_id, posting_locs, bucket_name,folder_str)
        return bucket_id

    
    @staticmethod
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name,folder_str):
        with open(f"{bucket_id}_posting_locs.pickle", "wb") as f:
            pickle.dump(posting_locs, f)
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob_posting_locs = bucket.blob(f"postings_gcp/.{folder_str}/{bucket_id}_posting_locs.pickle")
    ...
    def get_posting_iter(self):
        ...
        """
        words, pls = zip(*self.posting_lists_iter())
        return words, pls
